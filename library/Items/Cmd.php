...
class Cmd
{
    ...
    public function run ($argv)
    {
        try {
            ...
            if (! file_exists(dirname(__FILE__) . "/Cmd/$class.php")) {
                throw new \Exception(sprintf('"%s" was not found in lazuli commands!', $class));
            }

            require_once dirname(__FILE__) . '/Cmd/' . $class . '.php';

            $c_name = 'Items\Cmd\\' . $class;
            $c = new $c_name;
    ...
    public function cmdList()
    {
        ...
        echo "\n" . pack('c',0x1B) . "[1m" . '-- items commands list --' . pack('c',0x1B) . "[0m" . "\n";
        ...
        if ($dh = opendir($path)) {
            while ($entry = readdir($dh)) {
                if ($entry != '.' && $entry != '..' && preg_match('/.*\.php/', $entry)) {
                    ...
                    $count = 30;
                    $c_name = 'Items\Cmd\\' . str_replace('.php', '', $entry);
